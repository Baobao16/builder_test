...
func Test_p0_bribe(t *testing.T) {
	...
	t.Run("bribe_success", func(t *testing.T) {
		...
		*/
		...
		txs, _ := utils.SendLockMempool(conf.RootPk, conf.Mylock, lockData, conf.MedGas, false, true)
		...
		usr2Arg := utils.UserTx(conf.RootPk3, conf.Mylock, testcase.UnlockStrData, conf.MedGas)
...
func Test_p0_SpecialOp(t *testing.T) {
	t.Run("testCoinbase", func(t *testing.T) {
		...
		arg := utils.UserTx(conf.RootPk, conf.SpecialOp, conf.SpecialOpCb, conf.HighGas)
		arg.TxCount = 1
	...
	t.Run("testTimestamp", func(t *testing.T) {
		...
		arg := utils.UserTx(conf.RootPk2, conf.SpecialOp, blkTS, conf.HighGas)
		arg.TxCount = 1
		txs, bundleArgs, _ := sendBundle.ValidBundle_NilPayBidTx_1(&arg)
		cbn := utils.SendBundlesMined(t, arg, bundleArgs)
		utils.WaitMined(txs, cbn)
	...
	t.Run("testTimestampEq_parent", func(t *testing.T) {
		...
		arg := utils.UserTx(conf.RootPk3, conf.SpecialOp, blkTS, conf.HighGas)
		arg.TxCount = 1
		txs, bundleArgs, _ := sendBundle.ValidBundle_NilPayBidTx_1(&arg)
		cbn := utils.SendBundlesMined(t, arg, bundleArgs)
		utils.WaitMined(txs, cbn)
	...
	t.Run("blkHash", func(t *testing.T) {
		...
		arg := utils.UserTx(conf.RootPk4, conf.SpecialOp, blkHash, conf.HighGas)
		arg.TxCount = 1
		txs, bundleArgs, _ := sendBundle.ValidBundle_NilPayBidTx_1(&arg)
		cbn := utils.SendBundlesMined(t, arg, bundleArgs)
		utils.WaitMined(txs, cbn)
...
func Test_p0_BundleBribe(t *testing.T) {
	...
	for index, tc := range testCases {
		t.Run("backRun_value_preservation_case"+strconv.Itoa(index), func(t *testing.T) {
			...
			usr2Arg := utils.UserTx(conf.RootPk2, conf.WBNB, conf.TransferWBNBCode, conf.HighGas)
